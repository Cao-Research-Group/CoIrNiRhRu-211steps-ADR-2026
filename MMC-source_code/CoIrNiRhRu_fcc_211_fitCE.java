...
import matsci.io.clusterexpansion.CLUST;
...
import matsci.structure.decorate.function.ce.reg.ExpExpRegGenerator;
import matsci.structure.decorate.function.ce.reg.ExpPowRegGenerator;
import matsci.structure.decorate.function.ce.reg.ExpPowRegGenerator2;
import matsci.structure.decorate.function.ce.reg.coupled.CongruenceCoupledRegGenerator;
 ...
 *
 */
public class CoIrNiRhRu_fcc_111_fitCE {
    ...
    public static String PRIM_S_FILE = ROOT_DIR + "/PRIM-S.vasp";
    public static String ENERGIES = ROOT_DIR + "/energies-list.txt";
    public static String GROUND_DIR = ROOT_DIR + "/groundstates/";
    public static String RANDOMSTRUCT_DIR = ROOT_DIR + "/randomstruct/";
    public static String FILELIST = ROOT_DIR + "/structuresList.txt";
    public static String NUMOFNIPTO = ROOT_DIR + "/numOfNiPtO.txt";
    //public static NumberFormat NUMBERFORMAT = NumberFormat.getNumberInstance();
    //public static String METASTABLES_DIR = ROOT_DIR + "/metastables-0.25ML/";


 
    /*public static double CoEnergyBulk = -6.4972828; // actually is Co
    public static double CuEnergyBulk = -3.37696175; // actually is Cu
    public static double FeEnergyBulk = -7.65274; // actually is Fe
    public static double MoEnergyBulk = -9.9386188; // actually is Mo
    public static double NiEnergyBulk = -4.9648155; // actually is Ni*/

    public static double CoEnergyBulk = -6.4972828; // actually is Co
    public static double IrEnergyBulk = -8.18628425; // actually is Cu
    public static double NiEnergyBulk = -4.9648155; // actually is Fe
    public static double RhEnergyBulk = -6.614891; // actually is Mo
    public static double RuEnergyBulk = -8.47847925; // actually is Ni

    public static double NEnergyCorrectAccurate = -8.179681165;
    //public static double COEnergyCorrectAccurate = 14.507834;
    //public static double O2Energy = -8.33654;

    // E(O2(g) = -9.713 eV
    // u(O2, T, p0) = 1.376 eV, correction
    // -8.33654 = -9.713 + 1.376
    ...
    public static void main(String[] args) {
        ...
        //checkNanoparticleRelaxation_driver(double relaxRatio, double cellRatio, String poscarDir, String contcarDir, String structListDir, String structList);

        //checkNanoparticleRelaxation_driver(0.75, 1.0, STRUCT_DIR + "/PREC=Norm-CONTCAR-finish-num=92-20230905/", STRUCT_DIR + "/PREC=Norm-CONTCAR-finish-num=92-20230905/", STRUCT_DIR, "structList-num=92-20230905-Normal-partial.txt");


        //replaceElementsAgAuCuPdPtWithCoCuFeMoNi("/structures/CoCuFeMoNi-structures/", "energies-list-20230918.txt", 2, 2) ;
        //replaceElementsCoCuFeMoNiWithCoIrNiRhRu("/structures/CoIrNiRhRu-structures/", "struct-N.txt", 3.697, 3.738) ;

        ClusterExpansion ce = buildCE();

        fitCE(ce);

        //simpleRelaxation_cleanslab(ROOT_DIR, ROOT_DIR,"detect", 0.5, 1);
    ...
    public static ClusterExpansion buildCE() {
        ...
        PRIM prim = new PRIM(PRIM_S_FILE);
        ...
        //ce.appendClusters(2, 4.044);
        //ce.appendClusters(2, 8);
        //ce.appendClusters(2, 10); //used for JPCC PtNi-O/Vacancy PRIM
        numNewClusters = ce.numGroups() - numTotalClusters;
        numTotalClusters = ce.numGroups();
        Status.basic("Added " + numNewClusters + " pair clusters");

        ce.appendClusters(3, 2.8);
        //ce.appendClusters(3, 0);
        //ce.appendClusters(3, 4); //used for JPCC PtNi-O/Vacancy PRIM
        //ce.appendClusters(3, 5);
        numNewClusters = ce.numGroups() - numTotalClusters;
        numTotalClusters = ce.numGroups();
        Status.basic("Added " + numNewClusters + " triple clusters");

        //ce.appendClusters(4, 4);
        //ce.appendClusters(4, 0);
        //ce.appendClusters(4, 4); //used for JPCC PtNi-O/Vacancy PRIM
        //ce.appendClusters(4, 5);
        numNewClusters = ce.numGroups() - numTotalClusters;
        numTotalClusters = ce.numGroups();
        Status.basic("Added " + numNewClusters + " quad clusters");
        /*
        ce.appendClusters(5, 3.8);
        //ce.appendClusters(4, 3);
        //ce.appendClusters(4, 4); //used for JPCC PtNi-O/Vacancy PRIM
        //ce.appendClusters(4, 5);
        numNewClusters = ce.numGroups() - numTotalClusters;
        numTotalClusters = ce.numGroups();
        Status.basic("Added " + numNewClusters + " quint clusters");
       */
        /**
        ...
        for (int clustNum = 0; clustNum < ce.numGroups(); clustNum++) {
          ...
          //outfile.writeVICSOutFile(CLUSTER_DIR + "cluster." + clustNum + ".out");
     ...
     * @return The created cluster expansion
     */
    public static ClusterExpansion buildCE_PtNiCu() {

      /**
       * The PRIM file defines primitive cell for the cluster expansion, including
       * which atoms are allowed at each site.
       */
      PRIM prim = new PRIM(ROOT_DIR + "/PRIM-PtNiCu.vasp");
      ...
      //ce.appendClusters(2, 8);
      ce.appendClusters(2, 10);
      numNewClusters = ce.numGroups() - numTotalClusters;
      numTotalClusters = ce.numGroups();
      Status.basic("Added " + numNewClusters + " pair clusters");

      ce.appendClusters(3, 4);
      //ce.appendClusters(3, 5);
      numNewClusters = ce.numGroups() - numTotalClusters;
      numTotalClusters = ce.numGroups();
      Status.basic("Added " + numNewClusters + " triple clusters");
      ...
      for (int clustNum = 0; clustNum < ce.numGroups(); clustNum++) {
        ...
        Status.detail("clustNum:" + clustNum + " , numSites:"
            ...
            + group.getMultiplicity()+", ECI:"
            + group.getECI(0));
        POSCAR outfile = new POSCAR(group.getSampleStructure(Species.get("Pt"), Species.get("Cu")));
        outfile.writeFile(CLUSTER_DIR + "cluster." + clustNum + ".vasp");
        outfile.writeVICSOutFile(CLUSTER_DIR + "cluster." + clustNum + ".out");
      }
    ...
    /**
     * Fits the ECI using the Bayesian cluster expansion.
     * See "A Bayesian Approach to Cluster Expansions" for details.
     *
     * @param ce
     */
    public static void fitCE(ClusterExpansion ce) {

      /**
       * This is the list of cluster groups for which we want to assign the ECI
       */
      ClusterGroup[] activeGroups = ce.getAllGroups(true);
      //ClusterGroup[] activeGroups = ClustersFilter(ce);

      /**
       * Read in all of the structures in the training set
       */
      addKnownStructuresFormationEnergy(ce);

      /**
       * These are the "X" matrix and "y" vector described in my articles.
       */
      StructureList list = ce.getStructureList();
      /**
       *  getMultCorrelations is used to fit "Vi" instead of "mi * Vi",
       *  where "Vi" is the ECi and "mi" is the number of symmetrically
       *  equivalent clusters per unit cell.
       */
      double[][] correlations = list.getMultCorrelations(activeGroups, true);
      double[] values = list.getValues(true);
  ...
  */
      /**
       * See the paper "Bayesian approach to cluster expansions" for a description of
       * how regularization matrix generators are used.  This one generates the "lambda_sub_alpha"
       * terms, based on exponential decay with respect to the number of sites per cluster and
       * distance between sites.
       */
      //1st "Exp/Pow": exp/power decreases as a function of distance within the cluster
      //2nd "Exp/Pow": exp/power decreases as a function of # of atoms
      //ExpExpRegGenerator selfGenerator = new ExpExpRegGenerator(activeGroups); // used in JPCC
      //ExpPowRegGenerator selfGenerator = new ExpPowRegGenerator(activeGroups); //works better in general
      //PowExpRegGenerator selfGenerator = new PowExpRegGenerator(activeGroups);

      //ExpExpRegGenerator2 selfGenerator = new ExpExpRegGenerator2(activeGroups);
      //ExpPowRegGenerator2 selfGenerator = new ExpPowRegGenerator2(activeGroups);
      ExpPowRegGenerator2 selfGenerator = new ExpPowRegGenerator2(activeGroups);
      //selfGenerator = ClustersFilter(ce);

      selfGenerator.getActiveGroups();
      
      selfGenerator.setMultiplicityExponent(0);

      /**
       * This is the generator used to create the lambda_sub_alphabeta values.
       * This uses a single parameter to scale all of the values, relative to the lambda_sub_alpha
       * values.  I find this often works well.
       */
      //BulkCoupledRegGenerator generator = new BulkCoupledRegGenerator(ce, selfGenerator, activeGroups);
      //CongruenceCoupledRegGenerator generator = new CongruenceCoupledRegGenerator(selfGenerator, activeGroups);
      CongruenceCoupledRegGenerator generator = new CongruenceCoupledRegGenerator(selfGenerator, activeGroups);

      //LambdaRegGenerator generator = new LambdaRegGenerator(activeGroups);
      generator.setMultiplicityExponent(0);
      double[] initState = new double[generator.numParameters()];

      System.out.println("Initial States");

      for (double element : initState) {

          System.out.println(element);
      }

      Arrays.fill(initState, 1); // For some reason this seems to be a good starting point in general
      ...
      initState[3] = 1.0;
      initState[4] = 1.0;*/
      initState[0] = 0.49850685857227445;
      initState[1] = 0.613507213147732;
      initState[2] = 0.5580633660514793;
      initState[3] = 1.6484972126500377;
      initState[4] = 0.7709233470303551;
      
      System.out.println("Initial States after filling '1' ");
      for (double element : initState) {
...
/*
      double[][] outputRegularMaxtrix = generator.getRegularizer(initState, null);
      System.out.println("initial Regularization function matrix: ");
      Status.detail("Regularization function:");
      Status.detail(DoubleFactory2D.dense.make(outputRegularMaxtrix).toString());
*/

      /**
       * This is a least squares fitter that accepts a regularization matrix as well.
       */
      //L2FullRLSFitter fitter = new L2FullRLSFitter(correlations, values, outputRegularMaxtrix);
      L2FullRLSFitter fitter = new L2FullRLSFitter(correlations, values, generator.getRegularizer(initState, null));

      fitter.useLKOCVScore(1);    // Leave-one-out cross-validation

      System.out.println(fitter);

      /**
       * This is where we start before optimizing parameters
       * NaN means "not a number"
       */
      Status.basic("************ Before optimizing score *********");
      Status.basic("Score: " + fitter.scoreFit() + ", ");
      Status.basic("GCV: " + fitter.getGeneralizedCVScore() + ", ");
      Status.basic("LOO CV: " + fitter.getLKOCVScore(1) + ", ");
      Status.basic("Unweighted LOO CV: " + fitter.getUnweightedLOOCVScore() + ", ");
      Status.basic("");

      /**
       * We use a conjugate gradient algorithm to find the optimal regularization parameters.
       *
       * You can reduce the max allowed gradient to get a lower CV score, but it takes longer.
       */
      RegularizerSelector selector = new RegularizerSelector(fitter, generator, initState);

      Status.basic("Initial States");
      for (double element : initState) {

          Status.basic(element+ "");
      }

      selector.setMaxAllowedGrad(1E-3);
      selector.findMinPolakRibiere(); //Polak-Ribiere is a kind of conjugate gradient algorithm

      /**
       *  From here on out it's just reporting
       */
      fitter = (L2FullRLSFitter) selector.getFitter();

      Status.basic("************ After optimizing score *********");
      Status.basic("Score: " + fitter.scoreFit() + ", ");
      Status.basic("GCV: " + fitter.getGeneralizedCVScore() + ", ");
      Status.basic("LOO CV: " + fitter.getLKOCVScore(1) + ", ");
      Status.basic("Unweighted LOO CV: " + fitter.getUnweightedLOOCVScore() + ", ");
      ...
      //Status.detail("Final Parameters: ");
      Status.basic("Final Parameters: ");
      for (double element : finalState) {
        //Status.detail(finalState[paramNum] + ", ");
          Status.basic(element + ", ");
      }
      Status.basic("");
      ...
      System.out.println(eci);// to see whether eci get memory

      System.out.println("the # of ECI are: " + eci.length);

      System.out.println("\nThe list of fitted ECIs: ");
      for (double element : eci) {
        //Status.detail(eci[varNum] + ", ");
          //Status.basic(eci[varNum] + ", ");
      ...
      for (int trainingStructNum = 0; trainingStructNum < list.numKnownStructures(); trainingStructNum++) {
        ClustStructureData structData = list.getKnownStructureData(trainingStructNum);
        double value = structData.getValue();
        double predValue = list.predictValue(activeGroups, trainingStructNum);
        double weight = fitter.getWeight(trainingStructNum);
        //Status.detail(structData.getDescription() + ", Known Value: " + value + ", Predicted value: " + predValue + ", CV value: " + (value - cvErrors[trainingStructNum]) + ", CV Error: " + cvErrors[trainingStructNum] + ", Weight: " + weight);
        Status.basic(structData.getDescription() + ", Known Value: " + value + ", Predicted value: " + predValue + ", CV value: " + (value - cvErrors[trainingStructNum]) + ", CV Error: " + cvErrors[trainingStructNum] + ", Weight: " + weight);
      }
      //Status.detail("");
      Status.basic("");

      // print the error info for structures that absolute value of CV Error is larger that 0.05
      Status.basic("print the error info for structures that absolute value of CV Error is larger that 0.05" + "\n");
      for (int trainingStructNum = 0; trainingStructNum < list.numKnownStructures(); trainingStructNum++) {
        ClustStructureData structData = list.getKnownStructureData(trainingStructNum);
        double value = structData.getValue();
        double predValue = list.predictValue(activeGroups, trainingStructNum);
        double weight = fitter.getWeight(trainingStructNum);
        if(Math.abs(cvErrors[trainingStructNum]) >= 0.05) {
            Status.basic(structData.getDescription() + ", Known Value: " + value + ", Predicted value: " + predValue + ", CV value: " + (value - cvErrors[trainingStructNum]) + ", CV Error: " + cvErrors[trainingStructNum] + ", Weight: " + weight);
        } // end of if
      }
      Status.basic("");

      // print the error info for structures that absolute value of CV Error is larger that 0.1
      Status.basic("print the error info for structures that absolute value of CV Error is larger that 0.1" + "\n");
      for (int trainingStructNum = 0; trainingStructNum < list.numKnownStructures(); trainingStructNum++) {
        ClustStructureData structData = list.getKnownStructureData(trainingStructNum);
        double value = structData.getValue();
        double predValue = list.predictValue(activeGroups, trainingStructNum);
        double weight = fitter.getWeight(trainingStructNum);
        if(Math.abs(cvErrors[trainingStructNum]) >= 0.1) {
            Status.basic(structData.getDescription() + ", Known Value: " + value + ", Predicted value: " + predValue + ", CV value: " + (value - cvErrors[trainingStructNum]) + ", CV Error: " + cvErrors[trainingStructNum] + ", Weight: " + weight);
        } // end of if
      }
      //Status.detail("");
   ...
   // print the error info for structures that absolute value of CV Error is larger that 0.25
      Status.basic("print the error info for structures that absolute value of CV Error is larger that 0.25" + "\n");
      for (int trainingStructNum = 0; trainingStructNum < list.numKnownStructures(); trainingStructNum++) {
        ClustStructureData structData = list.getKnownStructureData(trainingStructNum);
        double value = structData.getValue();
        double predValue = list.predictValue(activeGroups, trainingStructNum);
        double weight = fitter.getWeight(trainingStructNum);
        if(Math.abs(cvErrors[trainingStructNum]) >= 0.25) {
            Status.basic(structData.getDescription() + ", Known Value: " + value + ", Predicted value: " + predValue + ", CV value: " + (value - cvErrors[trainingStructNum]) + ", CV Error: " + cvErrors[trainingStructNum] + ", Weight: " + weight);
        } // end of if
      }
      //Status.detail("");
      ...
      // print the error info for structures that absolute value of CV Error is larger that 0.5
      Status.basic("print the error info for structures that absolute value of CV Error is larger that 0.5" + "\n");
      for (int trainingStructNum = 0; trainingStructNum < list.numKnownStructures(); trainingStructNum++) {
        ClustStructureData structData = list.getKnownStructureData(trainingStructNum);
        double value = structData.getValue();
        double predValue = list.predictValue(activeGroups, trainingStructNum);
        double weight = fitter.getWeight(trainingStructNum);
        if(Math.abs(cvErrors[trainingStructNum]) >= 0.5) {
            Status.basic(structData.getDescription() + ", Known Value: " + value + ", Predicted value: " + predValue + ", CV value: " + (value - cvErrors[trainingStructNum]) + ", CV Error: " + cvErrors[trainingStructNum] + ", Weight: " + weight);
        } // end of if
      }
      //Status.detail("");
      ...
      for (int clustNum = 0; clustNum < ce.numGroups(); clustNum++) {
        ClusterGroup group = ce.getClusterGroup(clustNum);
        String description = "Group number " + clustNum + ", Sites per cluster: " + group.numSitesPerCluster() + ", Multiplicity: " + group.getMultiplicity() + ", Max distance: " + group.getMaxDistance() + ", ECI: " + group.getECI(0) + ", Correlation coefficient: " + group.getCorrelationCoefficient(0);
        //Status.detail(description);
        Status.basic(description);
      }

      Status.basic("\n\nprint out the clusters that contain nitrogen atom\n");

      for (int clustNum = 0; clustNum < ce.numGroups(); clustNum++) {
          ClusterGroup group = ce.getClusterGroup(clustNum);
          //Species[] sps = group.get
          boolean hasnitrogen = false;
          for(int clustSiteNum = 0; clustSiteNum < group.numSitesPerCluster(); clustSiteNum++) {
              Species[] allowedSpecies = group.getAllowedSpecies(clustSiteNum);
              if (ArrayUtils.arrayContains(allowedSpecies, Species.nitrogen)) {
                  hasnitrogen = true;
                  break;
              }
          }
          if (!hasnitrogen) {continue;}
          String description = "Group number " + clustNum + ", Sites per cluster: " + group.numSitesPerCluster() + ", Multiplicity: " + group.getMultiplicity() + ", Max distance: " + group.getMaxDistance() + ", ECI: " + group.getECI(0) + ", Correlation coefficient: " + group.getCorrelationCoefficient(0);
          //Status.detail(description);
          Status.basic(description);
        }
      ...
      Status.basic("\n\nprint out the clusters that do NOT contain nitrogen atom\n");
    ...
    /**
     * add structures, the properties corresponding to cluster expansions are formation energy
     *
     * @param ce
     */
   public static void addKnownStructuresFormationEnergy(ClusterExpansion ce) {

       Status.flow("Adding structures...");
       String path = STRUCT_DIR;


       /**
        * The StructureListFile reads in a simple file in which each line is just a file name
        * followed by the calculated energy per simulation cell.
        */
       StructureListFile strListFile = new StructureListFile(ENERGIES);
       //StructureListFile strListFile = new StructureListFile(ENERGIES_S0);

       /**
        * This is the object used by the cluster expansion to store all of the structures
        * in the training set.
        */
       StructureList list = ce.getStructureList();
       int numStructOneElement = 0;
       int numStructTwoElements = 0;
       int numStructThreeElements = 0;
       int numStructFourElements = 0;
       int numStructFiveElements = 0;

       try {
           //FileWriter fw = new FileWriter(ROOT_DIR + "/formationEnergies.txt" , false);
           FileWriter fw = new FileWriter(ROOT_DIR + "/formationEnergies-S.txt" , false);
           BufferedWriter bw = new BufferedWriter(fw);
           bw.write("fileName   formationEnergy     numPrim    Co%     Ir%     Ni%     Rh%     Ru%     N%" + "\r\n");

           /**
            * Go through all of the entries in the StructureListFile and add them to the StructureList
            */
           for (int entryNum = 0; entryNum < strListFile.numEntries(); entryNum++) {
             String fileName = strListFile.getFileName(entryNum);
             double energy = strListFile.getEnergy(entryNum);
             ...
             /**
              * Check to see if a structure file with the given filename exists.
              * This method assumes that the given structure files have POSCAR (VASP) file format.
              */
             String filePath = path + fileName;
             File inFile = new File(filePath);
             if (!inFile.exists()) {
               Status.warning("Couldn't find POSCAR for structure: " + fileName);
               continue;
             }

             /**
              * Read in the training structure from the file.  Because we are looking at surfaces,
              * we need to tell the cluster expansion that the structure is not periodic in the third
              * dimension.
              */
             POSCAR infile = new POSCAR(filePath, false);
             infile.setDescription(fileName);
             infile.setVectorPeriodicity(2, false);

             /**
              * This method expresses the structure read in from the file as a superstructure of
              * the primitive structure used to define the cluster expansion (as defined in
              * the "PRIM" file).
              */
             SuperStructure structure = ce.getBaseStructure().getSuperStructure(infile);

             for (int siteNum = 0; siteNum < structure.numDefiningSites(); siteNum++) {
               Structure.Site site = structure.getDefiningSite(siteNum);
               if (site.getSpecies() != Species.nitrogen) {continue;}
               Structure.Site[] neighbors =structure.getNearbySitesWithSpecies(site.getCoords(), Species.nitrogen, 2, false);
               if (neighbors.length > 0) {
                 System.out.println("!!!!! N too close !!!!! " + fileName);
                 break;
               }
             }
             ...
             /**
              *  The appliedCE is the object used to evaluate energies using a cluster expansions
              */
             FastAppliedCE appliedCE = new FastAppliedCE(ce, structure);
             //Structure str2= appliedCE.get
             appliedCE.refreshFromStructure(); // Read the site occupations from the structure used to create the appliedCE
             appliedCE.activateAllGroups(); // Include all known clusters
             appliedCE.getSuperStructure().setDescription(infile.getDescription());
             /*
             for (int sigmaSiteNum = 0; sigmaSiteNum < appliedCE.numSigmaSites(); sigmaSiteNum++) {
                 if(appliedCE.getSpecies(sigmaSiteNum)==Species.sulfur||appliedCE.getSpecies(sigmaSiteNum)==Species.vacancy) {
                 System.out.println(appliedCE.getSigmaIndex(sigmaSiteNum));
                 }
             }
             */

             /**
              * Add the structure.  Internally, the cluster expansion normalizes everything by
              * primitive cell as defined in the "PRIM" file, so we divide the given energy
              * by the number of primitive cells contained in the supercell used to construct the
              * structure in the training set.
              */
             Status.flow("Adding file " + fileName + ". the appliedCE.numPrimCells() is: " + appliedCE.numPrimCells());

             //Structure getStructure = appliedCE.getStructure();

             numCo = structure.numDefiningSitesWithSpecies(Species.cobalt);
             numIr = structure.numDefiningSitesWithSpecies(Species.iridium);
             numNi = structure.numDefiningSitesWithSpecies(Species.nickel);
             numRh = structure.numDefiningSitesWithSpecies(Species.rhodium);
             numRu = structure.numDefiningSitesWithSpecies(Species.ruthenium);
             numN = structure.numDefiningSitesWithSpecies(Species.nitrogen);
             numVacancy = structure.numDefiningSitesWithSpecies(Species.vacancy);
             
             int numMetalAtoms = numCo + numIr + numNi + numRh + numRu;
             //numN = structure.numDefiningSitesWithSpecies(Species.nitrogen);
             //numVacancy = structure.numDefiningSites() - numCu - numPt;
             numPrims = appliedCE.numPrimCells();

             double Co_comp = (1.0*numCo)/(numMetalAtoms) ;
             double Ir_comp = (1.0*numIr)/(numMetalAtoms) ;
             double Ni_comp = (1.0*numNi)/(numMetalAtoms) ;
             double Rh_comp = (1.0*numRh)/(numMetalAtoms) ;
             double Ru_comp = (1.0*numRu)/(numMetalAtoms) ;

             double S_comp = (1.0*numN)/(1.0*numN+1.0*numVacancy) ;
             ...
             switch (numElements) {
                 case 1:
                     ...
                     break;
                 case 2:
                     numStructTwoElements++;
                     break;
                 case 3:
                     numStructThreeElements++;
                     break;
                 case 4:
                     numStructFourElements++;
                     break;
                 case 5:
                     numStructFiveElements++;
                     break;
                 default:
                     System.out.println("ATTENTION! ERROR, the # of elements <1 or >5");
             }

             //System.out.println("the number of Pt, Ni, Vacancy, O is: " + numNi + ", "+ numNi + ", " + numVacancy + ", " + numO);

             formationEnergy = energy / numPrims - (numCo * CoEnergyBulk) / numPrims - (numIr * IrEnergyBulk) / numPrims - (numNi * NiEnergyBulk) / numPrims - (numRh * RhEnergyBulk) / numPrims - (numRu * RuEnergyBulk) / numPrims;

             formationEnergy = formationEnergy - ( numN * NEnergyCorrectAccurate) / numPrims;
             //formationEnergy = formationEnergy + ( numS * COEnergyCorrectAccurate) / numPrims;

             Status.flow("the formation energy of added structure is: " + formationEnergy);
             list.addCorrelations(appliedCE, formationEnergy); // Add the structure to the cluster expansion
             //list.addCorrelations(appliedCE, energy / appliedCE.numPrimCells());

             //write formation energy info to formationEnergies.txt file.
             NUMBERFORMAT.setMaximumFractionDigits(6);

             String stringNum = fileName + "   " + NUMBERFORMAT.format(formationEnergy) + "   " + numPrims + "   " + NUMBERFORMAT.format(Co_comp) + "   " + NUMBERFORMAT.format(Ir_comp)  + "   " + NUMBERFORMAT.format(Ir_comp) + "   " + NUMBERFORMAT.format(Ni_comp)  + "   " + NUMBERFORMAT.format(Rh_comp)  + "   " + NUMBERFORMAT.format(Ru_comp) + "   " + NUMBERFORMAT.format(S_comp) ;
             bw.write(stringNum+'\n');

           }

           Status.flow("the formationEnergies.txt file has been created and filled.");
           bw.flush();
           bw.close();
           fw.close();
       ...
       System.out.println("# of structures with 5, 4, 3, 2, and 1 elements are as follows: ");
       System.out.println("# of 5 elements: " + numStructFiveElements);
       System.out.println("# of 4 elements: " + numStructFourElements);
       System.out.println("# of 3 elements: " + numStructThreeElements);
       System.out.println("# of 2 elements: " + numStructTwoElements);
       System.out.println("# of 1 elements: " + numStructOneElement);

       System.out.println(list.numKnownStructures() + " structures added.");
     }
    ...
    * @return
    */
   public static void replaceElementsCoCuFeMoNiWithCoIrNiRhRu(String structDir_CoIrNiRhRu, String structList, double latticeParam_old, double latticeParam_new) {
	   //public static void replaceElementsAgAuCuPdPtWithCoCuFeMoNi(String structDir_CoCuFeMoNi, String structList, double latticeParam_old, double latticeParam_new) {

     //StructureListFile enerIn = new StructureListFile(ROOT + "energyList-PREC=Accu-finish.txt");
     StructureListFile enerIn = new StructureListFile(ROOT_DIR + structList);
     ...
     int numStructRh = 0;
     int numStructRu = 0;
     int numStructEmpty = 0;

     int numStructCount = 0;

     int numStructZeroElement = 0;
     int numStructOneElement = 0;
     int numStructTwoElements = 0;
     int numStructThreeElements = 0;
     int numStructFourElements = 0;
     ...
     for (int entryNum = 0; entryNum < enerIn.numEntries(); entryNum++) {
       ...
       POSCAR infile = new POSCAR(STRUCT_DIR + fileName);

       Structure structure = new Structure(infile);

       /*
        * int numAg = structure.numDefiningSitesWithElement(Element.silver);
       int numAu = structure.numDefiningSitesWithElement(Element.gold);
       int numCu = structure.numDefiningSitesWithElement(Element.copper);
       int numPd = structure.numDefiningSitesWithElement(Element.palladium);
       int numPt = structure.numDefiningSitesWithElement(Element.platinum);
       */

       int numCo = structure.numDefiningSitesWithElement(Element.cobalt);
       int numIr = structure.numDefiningSitesWithElement(Element.iridium);
       int numNi = structure.numDefiningSitesWithElement(Element.nickel);
       int numRh = structure.numDefiningSitesWithElement(Element.rhodium);
       int numRu = structure.numDefiningSitesWithElement(Element.ruthenium);

       int totalAtoms = numCo + numIr + numNi + numRh + numRu;

       int numN = structure.numDefiningSitesWithElement(Element.nitrogen);
/*
       if(numN!=0) {
           continue;
       }
       /*
        * read in empty.vasp, Pt.vasp, Ni.vasp, Mo.vasp and structures with more than 100 atoms
        */
       //if ((totalAtoms > 1) && (totalAtoms < 100)) {continue;}
       //if ((totalAtoms > 1) && (totalAtoms < 85)) {continue;}
       //if ((totalAtoms > 0) && (totalAtoms < 50)) {continue;}
       ...
       switch (numElements) {
           ...
           case 5:
               ...
               break;
           default:
               System.out.println("ATTENTION! ERROR, the # of elements <0 or >5");
       }
       ...
       for (int siteNum = 0; siteNum < totalAtoms; siteNum++) {
           Structure.Site site = structure.getDefiningSite(siteNum);

           if(site.getSpecies()==Species.nickel){ // if the surface site is sulfur, which stands for Pt-OH, then replace the S-atom with Pt-OH
               newSet_numRu++;
               site.setSpecies(Species.ruthenium); // replace Sulfur with Pt atom
           }
           else if(site.getSpecies()==Species.molybdenum){ // if the surface site is sulfur, which stands for Pt-OH, then replace the S-atom with Pt-OH
               newSet_numRh++;
               site.setSpecies(Species.rhodium); // replace Sulfur with Pt atom
           }
           else if(site.getSpecies()==Species.iron){ // if the surface site is sulfur, which stands for Pt-OH, then replace the S-atom with Pt-OH
               newSet_numNi++;
               site.setSpecies(Species.nickel); // replace Sulfur with Pt atom
           }
           else if(site.getSpecies()==Species.copper){ // if the surface site is sulfur, which stands for Pt-OH, then replace the S-atom with Pt-OH
               newSet_numIr++;
               site.setSpecies(Species.iridium); // replace Sulfur with Pt atom
           }
           else if(site.getSpecies()==Species.cobalt){ // if the surface site is sulfur, which stands for Pt-OH, then replace the S-atom with Pt-OH
               newSet_numCo++;
               site.setSpecies(Species.cobalt); // replace Sulfur with Pt atom
           }
       }



       structure = structure.scaleLatticeConstant(latticeParam_new / latticeParam_old );
       //structure = structure.scaleLatticeConstant((1+strain));

       //Species[] speciesInOrder_CoIrNiRhRu = new Species[] {Species.cobalt, Species.iridium, Species.nickel, Species.rhodium, Species.ruthenium};
       //POSCAR outfile = new POSCAR(structure, speciesInOrder_CoIrNiRhRu);


       Species[] speciesInOrder_Co_Ir_Ni_Rh_Ru_N = new Species[] {Species.cobalt, Species.iridium, Species.nickel, Species.rhodium,Species.ruthenium, Species.nitrogen};

       POSCAR outfile = new POSCAR(structure, speciesInOrder_Co_Ir_Ni_Rh_Ru_N);
       //String structName_new = fileName + ". numAg: " +  numAg + ", numAu: " + numAu + ", numCu: " + numCu + ", numPd: " + numPd + ", numPt: " + numPt;

       fileName = fileName.replaceAll("Ni", "Ru");
       fileName = fileName.replaceAll("Mo", "Rh");
       fileName = fileName.replaceAll("Fe", "Ni");
       fileName = fileName.replaceAll("Cu", "Ir");
       fileName = fileName.replaceAll("Co", "Co");

       outfile.writeFile(ROOT_DIR + structDir_CoIrNiRhRu + fileName);


     }


     Status.detail("CoCuFeMoNi replaced by CoIrNiRhRu: the # of " + numStructCount  + " structures.");


     System.out.println("\n# of structures with 5, 4, 3, 2, 1, and 0 elements are as follows: ");
     System.out.println("# of 5 elements: " + numStructFiveElements);
     System.out.println("# of 4 elements: " + numStructFourElements);
     System.out.println("# of 3 elements: " + numStructThreeElements);
     System.out.println("# of 2 elements: " + numStructTwoElements);
     System.out.println("# of 1 elements: " + numStructOneElement);
     System.out.println("# of Empty: " + numStructZeroElement + "\n");
   ...
   public static void checkNanoparticleRelaxation_driver(double relaxRatio, double cellRatio, String poscarDir, String contcarDir, String structListDir, String structList){
      ...
      for (int entryNum = 0; entryNum < enerIn.numEntries(); entryNum++) {
          ...
          /*
          int numNi = structure.numDefiningSitesWithElement(Element.platinum);
          int numNi = structure.numDefiningSitesWithElement(Element.nickel);
          ...
          */
          ...
          simpleRelaxation_cleanslab(poscarDir, contcarDir, fileName, relaxRatio, cellRatio);
  ...
  /**
   * this method used to decide whether the atoms of NP experience significant reconstruction upon relaxation.
   * defined as: an atom traveling more than 75% of the nearest-nearest distance from its initial site.
   * @param fileName
   * @return
   */
  public static boolean simpleRelaxation_cleanslab(String poscarDir, String contcarDir,String fileName, double relaxRatio, double cellRatio) {
      ...
      poscar.setVectorPeriodicity(0, true);
      poscar.setVectorPeriodicity(1, true);
      poscar.setVectorPeriodicity(2, false);
      contcar.setVectorPeriodicity(0, true);
      contcar.setVectorPeriodicity(1, true);
  ...
  public static void CountClusters(String PRIM, int numatoms, double maxdistance) {

      PRIM prim = new PRIM(PRIM);

      prim.setVectorPeriodicity(2, false);
      ClusterExpansion ce = new ClusterExpansion(prim);

      ce.appendClusters(numatoms, maxdistance);


      int[] numgroups = new int [100];
      double[] distgroups = new double [100];
      for (int clustNum = 1; clustNum < ce.numGroups(); clustNum++) {
    	  ClusterGroup group = ce.getClusterGroup(clustNum);
    	  int ind = 0;
    	  for (; distgroups[ind] != 0; ind++) {
    		  if (distgroups[ind] - group.getMaxDistance() < 0.0001 && group.getMaxDistance() - distgroups[ind] < 0.0001) {
				break;
			}
    	  }

    	  distgroups[ind] = group.getMaxDistance();
    	  numgroups[ind]++;

      }

      for (int ind = 0; distgroups[ind] != 0; ind++) {
		  System.out.println("maxdistance:" + distgroups[ind] + ", numclusters:" + numgroups[ind]);
	  }

	}

  public static ClusterGroup[] ClustersFilter(ClusterExpansion ce) {
      ClusterGroup[] activeGroups = ce.getAllGroups(true);
      System.out.println("\n\nPrint out the size of activeGroups before filter:");
      System.out.println(" activeGroups .length=" + activeGroups .length);
      System.out.println("\n\n");

      double[] distances = new double[activeGroups.length];
      for (int groupNum = 0; groupNum < distances.length;groupNum++) {
        distances[groupNum] =activeGroups[groupNum].getMaxDistance();
      }
      int[] map = ArrayUtils.getSortPermutation(distances);
      //sort the group & print the map of indices between non-sorted and sorted cluster list
      System.out.println("sorted_clust_index vs vasp_clust_index\n");
      ClusterGroup[] sortedGroups = new ClusterGroup[activeGroups.length];
      for (int groupNum = 0; groupNum <sortedGroups.length; groupNum++) {
        sortedGroups[groupNum] =activeGroups[map[groupNum]];
        System.out.println("sorted_clust_index, " + groupNum + ", vasp_clust_index, " + map[groupNum]);
      }
      System.out.println("\n");
      System.out.println("\n");

      //TODO
      //update the activeGroups, remove clusters 

      /**
       * JPCC cutoff (1, 10, 4, 4)
       * 
       * new cutoff  (1. 10, 6, 5), remove the addtional clusters without oxygen atoms.
       * 
       */

      ClusterGroup[] activeGroups_new = new ClusterGroup[0]; //activeGroups_new contains JPCC + pure-oxygen clusters with enhanced cutoff distances
      int[] activeGroups_new_Indices = new int[0];
      int numIncludedClusters=0;
      System.out.println("Print out information about the generated structures of clusters");
      for (int clustNum = 0; clustNum < sortedGroups.length; clustNum++) {
        ClusterGroup group = sortedGroups[clustNum];
        //ClusterGroup group = ce.getClusterGroup(clustNum);
        Status.detail("clustNum:" + clustNum + " , numSites:" 
            + group.numSitesPerCluster() + ", maxdistance:" 
            + group.getMaxDistance() + ", multiplicity:" 
            + group.getMultiplicity()+", ECI:"
            + group.getECI(0));
        int numSitesPerCluster = group.numSitesPerCluster();
        double maxDistance = group.getMaxDistance();

        if(numSitesPerCluster<=1) {
            numIncludedClusters++;
            activeGroups_new = (ClusterGroup[])ArrayUtils.appendElement(activeGroups_new, group);
            activeGroups_new_Indices = (int[])ArrayUtils.appendElement(activeGroups_new_Indices, clustNum);
        }
        else if(numSitesPerCluster==2) {
        	int AllowedSites =0;
            for(int clustSiteNum=0; clustSiteNum<numSitesPerCluster; clustSiteNum++) {
                Species[] spec_list = group.getAllowedSpecies(clustSiteNum);
                for(int specIndex =0; specIndex< spec_list.length; specIndex++) {
                    Species specLocal = spec_list[specIndex];
                    if((specLocal!=Species.nitrogen) && (specLocal!=Species.vacancy)) {
                        AllowedSites++;
                    }
                }
            }
        ...
        else if((numSitesPerCluster==3) || (numSitesPerCluster==4)) {
            if(maxDistance <=2.0) {
                numIncludedClusters++;
                activeGroups_new = (ClusterGroup[])ArrayUtils.appendElement(activeGroups_new, group);
                activeGroups_new_Indices = (int[])ArrayUtils.appendElement(activeGroups_new_Indices, clustNum);
      ...
      } // end of "for (int clustNum = 0; clustNum < activeGroups_old.length; clustNum++) "

      System.out.println("numIncludedClusters=" + numIncludedClusters);
      System.out.println("ce.numGroups())=" + ce.numGroups());
      System.out.println("activeGroups.length=" + sortedGroups.length);
      System.out.println("activeGroups_new.length=" + activeGroups_new.length);
      
      int numFunctionGroups = 0;
      for (int clustNum = 0; clustNum < activeGroups_new.length; clustNum++) {
        ClusterGroup group = ce.getClusterGroup(clustNum);
        //POSCAR outfile = new POSCAR(group.getSampleStructure(Species.get("Pt"), Species.get("Cu")));
        //outfile.writeVICSOutFile(CLUSTER_DIR + "cluster." + clustNum + ".out");
        numFunctionGroups += group.numFunctionGroups();
      }
      System.out.println("\n\n" + activeGroups_new.length + " cluster groups activated");
      System.out.println(numFunctionGroups + " function groups activated\n\n");

      //ce.
      //ce.appendClusterGroups(null)
      
      ExpPowRegGenerator2 selfGenerator = new ExpPowRegGenerator2(activeGroups_new); //works better in general
      return activeGroups_new;
  }
  ...
  public static ClusterExpansion getPreFittedCE_20250616_num920(){

      ClusterExpansion ce = buildCE(); //cutoff distances: (6, 3, 3)
      ClusterGroup[] activeGroups = ce.getAllGroups(true);
